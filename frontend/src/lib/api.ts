...
export const plaidApi = {
    ...
    getAccounts: async (): Promise<PlaidAccount[]> => {
        const response = await api.get<PlaidAccount[]>('/plaid/accounts');
        return response.data;
    },
    ...
    getTransactions: async (filters: TransactionFilters = {}): Promise<TransactionsResponse> => {
        ...
        const response = await api.get<TransactionsResponse>(`/plaid/transactions?${params.toString()}`);
        return response.data;
    },

    // Manually trigger transaction sync
    syncTransactions: async (): Promise<{ message: string }> => {
        const response = await api.post<{ message: string }>('/plaid/sync');
        return response.data;
    },
};
